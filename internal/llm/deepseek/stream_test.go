...
// Covers: text + reasoning interleaving, multi-fragment tool_call arguments
// across chunks, terminal usage frame, [DONE] terminator handling.
func TestConsumeStream(t *testing.T) {
	body := strings.Join([]string{
		...
		``,
		`data: {"choices":[],"usage":{"prompt_tokens":12,"completion_tokens":34,"prompt_cache_hit_tokens":5,"completion_tokens_details":{"reasoning_tokens":7}}}`,
		``,
		`data: [DONE]`,
		``,
	}, "\n")
	...
	wantChunks := []llm.Chunk{
		{Kind: llm.ChunkThinking, Delta: "hmm "},
		{Kind: llm.ChunkThinking, Delta: "thinking…"},
	...
	if got, want := resp.Thinking, "hmm thinking…"; got != want {
